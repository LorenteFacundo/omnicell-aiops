...
const servidor = new mcp_js_1.McpServer({
...
// =============================================================================
servidor.registerTool("query_cell_latencies", {
    description: "Consulta las latencias, estado de salud y métricas de rendimiento de todas las células del sistema OmniCell. " +
    ...
    inputSchema: zod_1.z.object({
        ...
        solo_celula: zod_1.z
            .string()
            .optional()
            .describe("ID de una célula específica para filtrar (ej: 'A', 'B', 'C'). Omitir para ver todas."),
    }),
}, 
...
async ({ solo_celula }) => {
    try {
        ...
        const snapshot = await gateway_client_js_1.gatewayClient.obtenerMetricas();
        ...
        lineas.push(`**Timestamp:** ${new Date(snapshot.timestamp).toLocaleString()}`);
        lineas.push(`**Throughput:** ${snapshot.requests_por_segundo.toFixed(1)} req/s`);
        lineas.push(`**Requests totales:** ${snapshot.total_requests.toLocaleString()}`);
        lineas.push(`**Tasa de error:** ${((snapshot.requests_fallidos / Math.max(snapshot.total_requests, 1)) * 100).toFixed(2)}%`);
        if (snapshot.hay_celulas_colapsadas) {
            const algunaActiva = celulas.some((c) => c.estado === "collapsed" && c.rango_max > c.rango_min);
            if (algunaActiva) {
        ...
        for (const celula of celulas) {
            const icono = celula.estado === "healthy"
                ? "🟢"
            ...
            lineas.push(`- Latencia promedio: ${celula.latencia_promedio_ms.toFixed(3)} ms`);
            lineas.push(`- Peticiones: ${celula.total_peticiones} (${celula.peticiones_fallidas} fallidas)`);
        ...
        lineas.push(`- Última pausa: **${snapshot.gc.ultima_pausa_ms.toFixed(3)} ms** (con BigCache off-heap, debe ser < 1ms)`);
        lineas.push(`- Pausa total acumulada: ${snapshot.gc.pausa_total_ms.toFixed(1)} ms`);
        lineas.push(`- Heap en uso: ${snapshot.gc.mem_heap_mb.toFixed(1)} MB`);
        lineas.push("\n### ⚡ Caché Off-Heap (BigCache)");
        lineas.push(`- Tasa de hit: **${snapshot.cache.tasa_hit_porcentaje.toFixed(1)}%**`);
        lineas.push(`- Entradas: ${snapshot.cache.cantidad_entradas}`);
        lineas.push(`- Memoria usada: ${snapshot.cache.capacidad_usada_mb.toFixed(1)} MB`);
        // Si hay alertas, agregamos recomendaciones de acción
        const celulasConAlerta = celulas.filter((c) => c.alerta);
        const rangoDrenado = (c) => c.rango_max <= c.rango_min;
        if (celulasConAlerta.length > 0) {
            lineas.push("\n### 🚨 Recomendaciones");
            for (const celula of celulasConAlerta) {
                if (celula.estado === "collapsed" && rangoDrenado(celula)) {
                    lineas.push(`- Célula **${celula.id}** está COLAPSADA pero su rango está drenado (tráfico ya redirigido). Estado mitigado. Opcional: recover cuando esté lista.`);
                }
                else if (celula.estado === "collapsed") {
                    lineas.push(`- Célula **${celula.id}** está COLAPSADA. Ejecutar \`rebalance_id_ranges\` incluyendo C (u otra) con rango drenado, p.ej. min=max sentínela.`);
                }
                else if (celula.estado === "degraded") {
                    lineas.push(`- Célula **${celula.id}** está DEGRADADA. Considerar ejecutar \`rebalance_id_ranges\` preventivamente.`);
                }
            }
        }
        return {
...
// =============================================================================
servidor.registerTool("rebalance_id_ranges", {
    description: "Redistribuye los rangos de IDs entre células para redirigir el tráfico de una célula colapsada o degradada. " +
    ...
    inputSchema: zod_1.z.object({
        ...
        nuevos_rangos: zod_1.z
            .record(zod_1.z.string(), zod_1.z.object({
            min: zod_1.z.number().describe("ID numérico mínimo del nuevo rango"),
            max: zod_1.z.number().describe("ID numérico máximo del nuevo rango"),
        }))
            .describe('Mapa de célula → nuevo rango. Incluir TODAS las células. Ejemplo al colapsar C: {"A":{"min":0,"max":1499999},"B":{"min":1500000,"max":2999999},"C":{"min":3000000,"max":3000000}}'),
        razon: zod_1.z
            .string()
            .optional()
    ...
    try {
        console.error(`[OmniCell MCP] Ejecutando rebalanceo. Razón: ${razon ?? "no especificada"}`);
        const resultado = await gateway_client_js_1.gatewayClient.rebalancearRangos(nuevos_rangos);
        const lineas = [];
        ...
        for (const [celula, rango] of Object.entries(nuevos_rangos)) {
            lineas.push(`- **Célula ${celula}:** IDs ${rango.min.toLocaleString()} → ${rango.max.toLocaleString()}`);
        }
...
// =============================================================================
servidor.registerTool("simulate_bulkhead_collapse", {
    description: "Simula el colapso de una célula de base de datos para demostrar la resiliencia del sistema. " +
    ...
    inputSchema: zod_1.z.object({
        ...
        tipo: zod_1.z
            ...
            .describe("'collapse': fallo catastrófico total | 'degrade': alta latencia | 'recover': restaurar a healthy"),
    }),
    ...
    try {
        ...
        switch (tipo) {
            case "collapse":
                resultado = await gateway_client_js_1.gatewayClient.colapsarCelula(celula_id);
                icono = "💥";
                descripcion = "COLAPSADA — no responde a peticiones";
                break;
            case "degrade":
                resultado = await gateway_client_js_1.gatewayClient.degradarCelula(celula_id);
                icono = "⚠️";
                descripcion = "DEGRADADA — responde con alta latencia";
                break;
            case "recover":
                resultado = await gateway_client_js_1.gatewayClient.recuperarCelula(celula_id);
                icono = "✅";
        ...
        if (tipo === "collapse") {
            ...
            lineas.push("Los compartimentos herméticos están activados. Las células A y B continúan operando.");
            lineas.push("\n**Próximos pasos recomendados:**");
            lineas.push("1. Ejecutar `query_cell_latencies` para confirmar el impacto");
            lineas.push("2. Ejecutar `rebalance_id_ranges` para redistribuir el tráfico de la Célula C");
        }
...
// =============================================================================
servidor.registerTool("get_system_health", {
    description: "Obtiene un resumen ejecutivo del estado de salud del sistema OmniCell. " +
    ...
    try {
        ...
        const rangoDrenado = (c) => c.rango_max <= c.rango_min;
        const celulasColapsadas = snapshot.celulas.filter((c) => c.estado === "collapsed");
        const colapsadasActivas = celulasColapsadas.filter((c) => !rangoDrenado(c));
        const colapsadasMitigadas = celulasColapsadas.filter((c) => rangoDrenado(c));
        const celulasDegradadas = snapshot.celulas.filter((c) => c.estado === "degraded");
        let estadoGeneral;
        let iconoEstado;
        ...
        const tasaError = snapshot.total_requests > 0
            ...
            : "0.00";
        const lineas = [];
        lineas.push(`## ${iconoEstado} Sistema OmniCell — Estado: **${estadoGeneral}**`);
        lineas.push(`**Throughput:** ${snapshot.requests_por_segundo.toFixed(1)} req/s | **Tasa de error:** ${tasaError}%`);
        // Resumen de células
        lineas.push("\n| Célula | Estado | Latencia p99 | Alerta |");
        lineas.push("|--------|--------|-------------|--------|");
        for (const celula of snapshot.celulas) {
            const estado = celula.estado === "healthy"
                ? "🟢 healthy"
                : celula.estado === "degraded"
                    ? "🟡 degraded"
                    : "🔴 collapsed";
            const alerta = celula.alerta ? "⚠️ Sí" : "✅ No";
            lineas.push(`| ${celula.id} | ${estado} | ${celula.latencia_p99_ms.toFixed(2)} ms | ${alerta} |`);
        }
        if (colapsadasActivas.length > 0) {
            lineas.push(`\n⚠️ **Células colapsadas con tráfico activo: ${colapsadasActivas.map((c) => c.id).join(", ")}**`);
            lineas.push("Ejecutar `rebalance_id_ranges` (incluir la célula colapsada con rango drenado) para redistribuir el tráfico.");
        }
        else if (colapsadasMitigadas.length > 0) {
            lineas.push(`\n🟠 **Mitigado:** células colapsadas sin tráfico (rango drenado): ${colapsadasMitigadas.map((c) => c.id).join(", ")}`);
            lineas.push("El rebalanceo ya redirigió el tráfico. Opcional: `simulate_bulkhead_collapse` con tipo `recover` cuando la célula esté lista.");
        }
        return {
            content: [{ type: "text", text: lineas.join("\n") }],
        };
